...
def plot_ppi_network_for_only_essential_genes(G):
    ...
    def scale_weight(w):
        return 1 + ((w - min_weight) / (max_weight - min_weight)) * 5  # Scale to range 1-6

    edge_weights = [scale_weight(G[u][v]['weight']) for u, v in G.edges()]
...
if __name__ == "__main__":
    ...
    ppi_data = get_string_interactions(["EGFR", "TGFB1", "VEGFA", "CDH2", "FGF18", "TFRC", "YWHAZ", "HMBS"])
    #print(ppi_data)
    ppi_network = create_network_from_string(ppi_data)
    print("Nodes:", ppi_network.nodes())
    print("Edges:", ppi_network.edges())
    plot_ppi_network_for_only_essential_genes(ppi_network)
